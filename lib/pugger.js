...
function render(route, filePath, data) {
    ...
    if (!cache) {
        ...
        if (!exist) {
            throw new SPError('NO_TEMPLATE_FILE', 'no template file: ' + filePath)
        } else {
            try {
    ...
    try {
        ...
        if (typeof this.config.global === 'function') {
            g_data = this.config.global()
        } else {
            g_data = this.config.global
        }

        // var rendered = cache(_.extend(this.config.global, data || {}))
        var rendered = cache(_.extend(g_data, data || {}))
    } catch (ex) {
        logger.call(this, 'error', fields, ex)
        throw ex
    }
